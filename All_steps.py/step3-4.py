...
class TrainEnergyOptimizer:
    ...
    def plot_results(self, results: Dict):
        ...
        # 速度-時間圖
        plt.subplot(121)
        plt.plot(time_s, speed_mps * 3.6, 'b-', label='最佳化速度')
        plt.xlabel('時間 (秒)')
        plt.ylabel('速度 (公里/小時)')
        plt.title('最佳化速度-時間圖')
        plt.legend()
        ...
        # 速度-距離圖
        distance_m = np.cumsum(speed_mps * np.diff(time_s, prepend=0))
        plt.subplot(122)
        plt.plot(distance_m, speed_mps * 3.6, 'r-', label='最佳化速度')
        plt.xlabel('距離 (公尺)')
        plt.ylabel('速度 (公里/小時)')
        plt.title('最佳化速度-距離圖')
        plt.legend()
